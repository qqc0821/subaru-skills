---
name: subaru-slides
description: 从内容到成品PPTX的端到端演示文稿制作，含AI插画生成和18种设计风格。当用户提到“做PPT”“做幻灯片”“演示文稿”“Keynote”或“slides”时使用。
---
...
> ⚠️ 以下风格 **强烈建议使用 Path A（HTML→PPTX）**。它们依赖精确排版、数据可视化和网格系统，AI图片生成无法达到所需精度。行业分析与数据报告类 deck 适合使用 Path A + Pentagram 编辑风格。
...
**更深入的风格细节**：优先使用本目录的设计参考；如果当前环境另有设计系统 skill，再按需读取其风格参考，不把外部 skill 当作必需依赖。
...
**Illustration Generation** — use the Codex `imagegen` capability when illustrations are needed. It produces bitmap assets directly; do not require a provider-specific API key or another repository skill.

```bash
# Invoke the imagegen capability with this prompt and save the returned asset:
# [description], 2048x1152 (16:9), no unintended text
```
...
**Generation request** (send the full-slide prompt to `imagegen`):

```text
Use imagegen with: [full slide prompt], 2048x1152 (16:9).
Save the returned image as slide-[NN]-[name].png.
```
...
### 4-A: Editable HTML Workflow (Path A)

Use the available `$presentations` skill for HTML-to-editable-PPTX conversion. Keep all helper paths resolved by that skill; never assume a home-directory installation. If `$presentations` is unavailable, use Path B so the deck can still be assembled with the bundled Python helper.

**HTML rules** (for the presentations workflow):
...
Assemble generated slide images into PPTX using this skill's `scripts/create_slides.py`:
...
**Path A:** Ask `$presentations` to preview 3-4 key HTML slides, or use a locally available Playwright executable:
```bash
...
**Detailed references:**
- `references/dependencies.md` — runtime requirements and dependency boundaries
...
| Skill | Role |
|-------|------|
| `presentations` | Editable PPTX assembly and preview for Path A, when available |
| `imagegen` | Optional bitmap illustration generation for either path |
...
> Subaru Slides · Project-local presentation workflow
