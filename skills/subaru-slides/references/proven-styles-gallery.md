...
> 实战验证的风格画廊。将同一页内容放入多种视觉系统进行对比，以便在真实 deck 中选择风格。
> 本项目为 Neo-Brutalism 风格保留可复用的设计规则与样例。
...
- 关键：**不要**写 "NOT Snoopy" 这类反向约束——描述情绪与世界观，让 AI 自由发挥（详见 proven-styles-snoopy.md）
...
**视觉参考：** Gumroad 官网、Figma 社区模板与当代品牌设计
**验证建议：** 在当前项目中用 HTML→PPTX 路径验证完整 deck。
...
**实战验证**：Pentagram Editorial 适合在本项目工作目录中以 HTML→PPTX 路径验证。
**更深入的风格细节**：优先参考本 skill 的 `design-principles.md` 与 `design-movements.md`；外部设计 skill 仅作为可选增强。

---

## 完整Prompt模板存档

第二轮扩展风格的可复制 prompt 已按需整理在本目录的 `prompt-templates.md`。

Snoopy风格的详细per-slide模板，详见：
`proven-styles-snoopy.md`

Neo-Brutalism 的完整 HTML 模板不作为外部文件依赖；需要时按本 skill 的设计规则在项目工作目录生成。
