...
clean professional aesthetic, 16:9 aspect ratio, 2048x1152
```
...
minimal data-visualization aesthetic, 16:9, 2048x1152
```
...
professional infographic style, 16:9, 2048x1152
```
...
friendly approachable aesthetic, 16:9, 2048x1152
```
...
The canonical base-style prompts live in the style presets: one per style under `../styles/<id>.md`
(registry: `../styles/index.json`). Do not duplicate them here.
For the method, see `illustrations.md`.
...
Generate slides in batches of 3-5 concurrently when the active image-generation capability supports parallel requests. Save each returned bitmap as `slide-[NN].png`.
